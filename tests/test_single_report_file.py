...
def main():
    ...
    with open(log_path, 'w', encoding='utf-8') as log:
        ...
        log.write(f"    start_page: {test_section['start_page']}\n")
        log.write(f"    end_page: {test_section['end_page']}\n")
        ...
        # 截取前 10000 字符，确保包含正文内容
        extracted_text = content[:10000]
        log.write(f"  截取文本长度: {len(extracted_text)} 字符\n")
        ...
        log.write(f"\n  [字段提取结果汇总]\n")
        log.write(f"    source_page (from section_locations): {start_page}\n")

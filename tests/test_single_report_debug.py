...
def main():
    ...
    try:
        ...
        print(f"\n  测试记录详情:")
        print(f"    doc_id: {test_section['doc_id']}")
        print(f"    issue_name: {test_section['issue_name']}")
        print(f"    section_title: {test_section['section_title'][:80]}...")
        print(f"    start_page: {test_section['start_page']}")
        print(f"    end_page: {test_section['end_page']}")
    ...
    print("\n[截取的文本片段]")
    print("-" * 60)
    preview_length = min(1500, len(extracted_text))
    ...
    try:
        ...
        print(f"\n  [字段提取结果汇总]")
        print(f"    source_page (from section_locations): {start_page}")
        print(f"    source_page (in result): {rule_result['source_page']}")
        ...
        print(f"    evidence_snippet (前200字): {rule_result['evidence_snippet'][:200]}...")
    except Exception as e:
        print(f"    规则字段提取失败: {e}")
        import traceback
        traceback.print_exc()
    ...
    output_record = {
        ...
        'source_page': rule_result['source_page'],  # 使用提取结果中的 source_page
